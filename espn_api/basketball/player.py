...
class Player(object):
    ...
    def __init__(self, data, year, pro_team_schedule = None, news = None):
        ...
        self.prev_average_stats = {name: -1 for name in STATS_MAP.values() if name != ''}
        self.total_stats = {name: -1 for name in STATS_MAP.values() if name != ''}
        ...
        self.nine_cat_prev_averages = {name : 0 for name in NINE_CAT_STATS}
        ...
        self.nine_cat_total = {name : 0 for name in NINE_CAT_STATS}
        ...
        for split in  player.get('stats', []):
            ...
            if split['seasonId'] == year:
                ...
                if id == f'{year}_total' and split.get('averageStats'):
                    self.total_stats = {
                            name: split['averageStats'].get(stat_id, -1)
                            for stat_id, name in STATS_MAP.items()
                            if name != ''
        ...
        self.nine_cat_prev_averages = self.combined_helper(sources1)
        ...
        self.nine_cat_total = self.combined_helper(sources5)
